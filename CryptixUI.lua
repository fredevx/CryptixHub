--==================================================
--  CryptixUI (Based on WindUI v1.6.57)
--  Version: v1.4.1
--  Default Name: CryptixHub
--  Default Theme: Dark
--  Author: fredevx
--==================================================
...
local base = "CryptixUI"
local hub = base .. "/CryptixHub"
EnsureFolder(base)
EnsureFolder(hub)
EnsureFolder(hub .. "/config")
EnsureFolder(hub .. "/mods")
--==================================================
--  CryptixUI (Based on CryptixUI v1.6.57)
--  Author: fredevx
--  Core UI Engine for CryptixHub
--  Includes:
--     * AnimEngine
--     * Dock System
--     * Themes
...
--[[
    ...
    Github: https://github.com/Footagesus/CryptixUI
...
local A=u.Object:FindFirstChild"CryptixUIGradient"
if A then
A:Destroy()
end
...
local L="CryptixUI/"..B.."/assets/."..C.."-"..v..".png"
...
warn(string.format("[ CryptixUI.Creator ] Failed to load custom asset '%s': %s",L,tostring(N)))
J:Destroy()
...
if not M then
warn("[ CryptixUI.Creator ]  '"..identifyexecutor().."' doesnt support the URL Images. Error: "..N)
...
return[[
{
    ...
    "version": "1.6.57",
    "main": "./dist/main.lua",
    "repository": "https://github.com/Footagesus/CryptixUI",
    "discord": "https://discord.gg/ftgs-development-hub-1300692552005189632",
    "author": "Footagesus",
...
local aj=a.load'l'.Init(nil,ag.CryptixUI.ScreenGui.KeySystem)
...
local h=ag.CryptixUI.Services[g.Type]
if h then
local i={}
...
ab.Parent=game:GetService"Darking"
end
...
for ad,ae in pairs(game:GetService"Darking":GetChildren())do
register(ae)
end
...
local ag=a.load'l'.Init(nil,ae.CryptixUI.ScreenGui.Popups)
...
warn"[ CryptixUI.ConfigManager ] Window.Folder is not specified."
return false
end

ab=ae
ac.Folder=ab.Folder
ac.Path="CryptixUI/"..tostring(ac.Folder).."/config/"

if not isfolder("CryptixUI/"..ac.Folder)then
makefolder("CryptixUI/"..ac.Folder)
if not isfolder("CryptixUI/"..ac.Folder.."/config/")then
makefolder("CryptixUI/"..ac.Folder.."/config/")
end
end
...
warn"[ CryptixUI.ConfigManager ] The config system doesn't work in the studio."
return nil
end
...
Parent=al.CryptixUI.DropdownGui,
AnchorPoint=Vector2.new(1,0),
},{
am.UIElements.Menu,
...
ay.UIElements.TabItem.HighDark.ImageTransparency=.75
...
ak(ay.UIElements.TabItem.HighDark,0.1,{ImageTransparency=.75}):Play()
...
ak(ay.UIElements.TabItem.HighDark,0.1,{ImageTransparency=1}):Play()
...
ak(aB.UIElements.TabItem.HighDark,0.1,{ImageTransparency=1}):Play()
...
local function getHighDark(al,am)
local an=al[am]
...
aq.Load(al,ap.Content,aq.Elements,ak.Window,ak.CryptixUI,function()
...
warn("[ CryptixUI ] Failed to apply pending config for '"..ar.Flag.."': "..tostring(ay))
end
end)
end
end
...
if ao.Elements[az].HighDark then
ao.Elements[az]:HighDark()
...
ao.ElementsModule.Load(ao,ao.UIElements.ContainerFrame,ao.ElementsModule.Elements,Window,CryptixUI,nil,ao.ElementsModule,an)
...
if ar.Folder then
if not isfolder("CryptixUI/"..ar.Folder)then
makefolder("CryptixUI/"..ar.Folder)
end
if not isfolder("CryptixUI/"..ar.Folder.."/assets")then
makefolder("CryptixUI/"..ar.Folder.."/assets")
end
...
if not g then
warn("[ CryptixUI.Window.Background ] Failed to download video: "..tostring(h))
return
end
end
...
if not g then
warn("[ CryptixUI.Window.Background ] Failed to load custom asset: "..tostring(h))
return
end
warn"[ CryptixUI.Window.Background ] VideoFrame may not work with custom video"
aE=h
end
...
local j=0
local l=ar.UIElements.Main.Main.Topbar.Right.UIListLayout.AbsoluteContentSize.X/aq.CryptixUI.UIScale
if i and g then
j=math.max(i.TextBounds.X/aq.CryptixUI.UIScale,g.TextBounds.X/aq.CryptixUI.UIScale)
else
j=i.TextBounds.X/aq.CryptixUI.UIScale
end
if h then
j=j+(ar.IconSize/aq.CryptixUI.UIScale)+(ar.UIPadding/aq.CryptixUI.UIScale)+(4/aq.CryptixUI.UIScale)
end
ar.UIElements.Main.Main.Topbar.Center.Position=UDim2.new(
0,
j+(ar.UIPadding/aq.CryptixUI.UIScale),
0.5,
0
)
ar.UIElements.Main.Main.Topbar.Center.Size=UDim2.new(
1,
-j-l-((ar.UIPadding*2)/aq.CryptixUI.UIScale),
1,
0
)
end)
...
ImageTransparency=ar.Transparent and aq.CryptixUI.TransparencyValue or 0
},{
l
})
end
...
aq.CryptixUI.TransparencyValue=p
...
if aq.CryptixUI.UseAcrylic then
...
aq.CryptixUI:ToggleAcrylic(true)
end)
end)
end
function ar.Close(p)
...
aq.CryptixUI:ToggleAcrylic(false)
...
ar.Destroyed=true
task.wait(0.4)
aq.CryptixUI.ScreenGui:Destroy()
aq.CryptixUI.NotificationGui:Destroy()
aq.CryptixUI.DropdownGui:Destroy()
...
ar.Transparent=r
aq.CryptixUI.Transparent=r

ar.UIElements.Main.Background.ImageTransparency=r and aq.CryptixUI.TransparencyValue or 0
...
return aq.CryptixUI.UIScale
end

function ar.SetUIScale(p,r)
aq.CryptixUI.UIScale=r
ak(aq.CryptixUI.ScreenGui.UIScale,.2,{Scale=r},Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
return ar
end
...
local p=a.load'T'
local r=a.load'U'
local u=p.Init(ar,aq.CryptixUI,aq.Parent.Parent.ToolTips)
...
return u.New(A,aq.CryptixUI.UIScale)
end
...
return r.New(A,ar.UIElements.SideBar.Frame,ar.Folder,aq.CryptixUI.UIScale,ar)
end
...
local R=J.AbsoluteContentSize.X/aq.CryptixUI.UIScale
local S=L.AbsoluteSize.X/aq.CryptixUI.UIScale
...
local Y=X.AbsoluteSize.X/aq.CryptixUI.UIScale
if Y<V then
V=Y
U=X
end
...
Name="CryptixUI/Notifications",
Parent=aq,
IgnoreGuiInset=true,
})
ac.DropdownGui=al("ScreenGui",{
Name="CryptixUI/Dropdowns",
Parent=aq,
IgnoreGuiInset=true,
})
ap(ac.ScreenGui)
...
if not isfolder"CryptixUI"then
makefolder"CryptixUI"
end
if at.Folder then
